...
namespace Northwind.DataAccess
{
    public class OrderRepository : Repository<Order>, IOrderRepository
    {
        ...
        public IEnumerable<OrderList> getPaginatedOrder(int page, int rows)
        {
            ...
            {
                var reader= connection.QueryMultiple("dbo.get_paginated_orders", parameter, commandType: System.Data.CommandType.StoredProcedure);
        ...
        public OrderList GetOrderById(int orderId) {
            ...
            using (var connection = new SqlConnection(_connectionString)) {
                ...
                var orderList = reader.Read<OrderList>().ToList();
                var orderListItem = reader.Read<OrderItemList>().ToList();

                foreach (var item in orderList) item.SetDetails(orderListItem);
